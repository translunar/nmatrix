...
Two gems are provided: the nmatrix gem which provides core features, and the nmatrix-atlas gem which extends the nmatrix gem and provides advanced linear algebra operations via ATLAS. Available from the <tt>atlas_plugin</tt> branch of https://github.com/wlevine/nmatrix.git

If you want to build (or use) the nmatrix-atlas gem, you will need to install ATLAS, {here is some help with that}[https://github.com/SciRuby/nmatrix/wiki/Installation].

If you don't want the nmatrix-atlas gem, you don't need to worry about installing ATLAS, all the dependencies should be handled by bundler:

    bundle install

To build both plugins for testing and developement:

    rake compile nmatrix_plugins=atlas

To run the specs:

    rake spec nmatrix_plugins=atlas

To install both gems:

    rake install nmatrix_plugins=atlas

Note that if you already have a previous version of nmatrix installed, you should run <tt>gem uninstall nmatrix</tt> before trying to install a new version. There seems to be a bug in rubygems that prevents the new installation from completely overwriting the old installation.

If you only want to test the nmatrix gem, and not nmatrix-atlas, use the same commands, but omit the <tt>nmatrix_plugins=atlas</tt>.
