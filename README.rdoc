...
Two gems are provided: the nmatrix gem which provides core features, and the nmatrix-atlas gem which extends the nmatrix gem and provides advanced linear algebra operations via ATLAS. Available from the `atlas_plugin` branch of https://github.com/wlevine/nmatrix.git
...
If you only want to test the nmatrix gem, and not nmatrix-atlas, use the same commands, but omit the +nmatix_plugins=atlas+
